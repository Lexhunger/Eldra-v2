// TCA9554PWR IO expander base driver: minimal init + set/get helpers.
#include <stdbool.h>
#include "esp_err.h"
...
esp_err_t EXIO_Init(void)
{
    ...
    esp_err_t ret = exio_i2c_init();
    if (ret != ESP_OK) {
        return ret;
    }

    TCA9554PWR_Init(0x00); // default all expander pins to outputs
    Write_REG(TCA9554_OUTPUT_REG, 0x00); // drive all outputs low (silence buzzer on EXIO8)
    initialized = true;
    return ESP_OK;
}
